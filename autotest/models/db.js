...
var db = new sqlite3.Database('./db/test.db3');
...
function insertResponseFlow(responseFlowVO, callback) {
  db
      .run(
          "insert into response_flow (id, flow_name, group_id_sequence, sequence_switch_request_path, flow_repeat) values (?,?,?,?,?);",
          [ uuid.v4(), responseFlowVO.flow_name, responseFlowVO.group_id_sequence,
              responseFlowVO.sequence_switch_request_path, responseFlowVO.flow_repeat ], function(error) {
            if (error) {
...
function updateResponseFlow(responseFlowVO, callback) {
  db
      .run(
          "update response_flow set flow_name=?, group_id_sequence=?, sequence_switch_request_path=?, flow_repeat=? where id=?;",
          [ responseFlowVO.flow_name, responseFlowVO.group_id_sequence, responseFlowVO.sequence_switch_request_path,
